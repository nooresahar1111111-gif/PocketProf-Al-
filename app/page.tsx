...
export default function Home() {
  ...
  const [messages, setMessages] = useState<Array<{ sender: string; text: string }>>([
    {
      sender: "ai",
      text: "Welcome to PocketProf AI! I am your 24/7 academic companion. Select your subject, upload notes or SNAP A PHOTO of any question paper, and I will solve it for you!"
    }
  ]);
  ...
  const handleStartNewChat = () => {
    ...
    setMessages([
      {
        sender: "ai",
        text: `New chat session started for [${selectedSubject}]. Ask questions or snap/upload question paper photos!`
      }
    ]);
  ...
  const handleDownloadSubjectData = async () => {
    ...
    setTimeout(() => {
      ...
      alert(`Downloaded offline study pack for "${selectedSubject}". All data stored locally!`);
    }, 1500);
  };
  ...
  const handleSafepayUpgrade = async () => {
    ...
    try {
      ...
      if (data.url) {
        ...
        alert("Payment initialization failed. Ensure SAFEPAY_API_KEY is configured in Vercel.");
      }
    } catch (err) {
  ...
  // Convert uploaded image to Base64 for Gemini Vision API
  ...
  const handleSendMessage = async (customText?: string) => {
    ...
    const userMsg = uploadedFile 
      ? `📷 [Image/File Attached: ${uploadedFile.name}] ${queryText}`
      : queryText;
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-slate-100 font-sans relative overflow-hidden">
      ...
      {/* Subject Control & Offline Download Bar */}
      <div className="p-3 bg-slate-900/60 border-b border-slate-800 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center bg-slate-800 border border-slate-700 rounded-lg overflow-hidden">
          <input
            ...
            className="bg-transparent text-xs px-2.5 py-1.5 text-slate-200 outline-none w-36"
          />
          <button
            onClick={handleAddSubject}
            className="bg-slate-700 hover:bg-slate-600 px-2 py-1.5 text-slate-200 border-l border-slate-700"
            title="Add Subject"
          >
        ...
        <button
          ...
          className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-medium px-3 py-1.5 rounded-lg ml-auto transition disabled:opacity-50"
        >
          <Download className="w-3.5 h-3.5" />
          {isDownloading ? "Downloading Data..." : "Download Offline Data"}
        </button>
      </div>

      {/* Quick Actions & Photo Uploader Bar */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {isAiLoading && (
          <div className="flex gap-3 max-w-xl items-center text-xs text-slate-400">
            ...
            <span>PocketProf is reading image & solving questions...</span>
          </div>
        )}
      </div>
